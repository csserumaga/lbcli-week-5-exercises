# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""

rd=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[0].scriptSig.asm | split(" ") | last')
address=$(bitcoin-cli -regtest decodescript "$rd" | jq -r '.segwit["p2sh-segwit"]')
echo "$address"
